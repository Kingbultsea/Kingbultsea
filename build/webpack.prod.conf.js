...
const ExtractTextPlugin = require('extract-text-webpack-plugin')// css文件和js文件要分开的插件
...
const webpackConfig = merge(baseWebpackConfig, {
  ...
  output: {
    ...
    filename: utils.assetsPath('js/[name].[chunkhash].js'),//编译输出文件格式
  ...
  plugins: [
    ...
    new UglifyJsPlugin({//压缩代码  去掉没有用的代码 实际上 你import 如果有其他无用代码 这个时候 就又这玩意来去掉 而不是webpack
      uglifyOptions: {
        compress: {
          warnings: false
        }
    ...
    new webpack.optimize.CommonsChunkPlugin({//Common Chunks插件能把你的Vendor代码（例如Vue.js这些不会经常改动的依赖包）和应用代码（每次开发过程中都会改动的代码）分离开。
      name: 'vendor',
      minChunks (module) {
